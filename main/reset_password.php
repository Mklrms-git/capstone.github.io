...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($new_password)) {
        ...
        if ($user_type === 'admin') {
            $verify_stmt = $conn->prepare("SELECT id FROM users WHERE id = ? AND username = ?");
        } else {
        ...
        if ($verify_result->num_rows === 1) {
            ...
            if ($user_type === 'admin') {
                $stmt = $conn->prepare("UPDATE users SET password = ? WHERE id = ? AND username = ?");
            } else {
...
<head>
    ...
    <title>Reset Password - Mhavis Medical & Diagnostic Center</title>
